...
export const handlers = [
  ...
  http.get("/api/todos", ({ request }) => {
    ...
    const splitData = request.url.split('?')[1]?.split('&').find(param => param.startsWith('page='));
    const page = splitData ? parseInt(splitData.split('=')[1]) : 1;
    const limit = parseInt(searchParams.get('limit') || '5');
    const sortField = searchParams.get('sortField') || 'id';
    const sortedData = JSON.parse(JSON.stringify([...todos])).sort((a: { [x: string]: number; }, b: { [x: string]: number; }) => {
      if (sortOrder === 'asc') {
